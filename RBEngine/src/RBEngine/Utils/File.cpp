...
namespace RB
{
	...
	File::File(FS::path path)
	{
		...
		{
			Debug::Log::Warn("Path cannot be set to a directory!");
			return;
		}
	...
	void File::Write(const char* data) const
	{
		...
		{
			Debug::Log::Error("Failed to create file! ({0})", ToString());
		}
	}
	...
	File File::GetFileDialog(const std::vector<DialogFilter>& filter)
	{
		...
		if (SUCCEEDED(result))
		{
			...
			if (SUCCEEDED(result))
			{
				result = pFileOpen->SetFileTypes(filter.size(), Utils::GetFilterSpec(filter).data());
				if (SUCCEEDED(result))
				{
	...
	std::ifstream File::_GetStream(bool binary) const
	{
		if (binary)
		{
			...
			if (!stream.is_open())
			{
				Debug::Log::Warn("Failed to open file {0}", m_Path.string());
			}
